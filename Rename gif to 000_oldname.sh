...
echo -------------------------========================-------------------------
	...
	echo 2023-03-24_Friday_11:28:17
	echo
## Software name, what is this, version, informations.
	echo "Software name: Convert XXX to"
	echo
	echo What it does ?
...
echo "Select folder or filename using dialog !"
	...
	file1=$(zenity --file-selection --filename=$HOME/ --title="Choose a directory to convert all file" --directory)	## Directory select.
	## file="/$HOME/Pictures/"
	## file="/$HOME/Downloads/"
	## --file-filter="*.jpg *.gif"
	## --file-filter='**[WwEeBbPp] | *[JjPpGg]' 

	count=`ls -1 "$file1" 2>/dev/null | wc -l`
	echo Count : $count
...
### file or folder
	if test -z "$file1"	## for cancel on zenity
		then
...
## directory section
	...
	echo
## Output file name
	name=`echo "$file1" | rev | cut -f 2- -d '.' | rev` ## remove extension
